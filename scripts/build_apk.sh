#!/usr/bin/env bash
# Build the English Word Coach APK end-to-end and verify the result.
set -euo pipefail
...
if [ ! -f data/final/words.json ]; then
  echo "[1/4] building dataset (network)…"
  python scripts/build_data.py
else
  echo "[1/4] dataset present, skipping fetch"
fi

# 2. package data into the web assets
echo "[2/4] packaging words.js…"
